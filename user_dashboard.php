...
<script>
const referralLink = <?php echo json_encode($referral_link, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>;

function toggleWalletBalance() {
    const displayDiv = document.getElementById('wallet_balance_display');
    const eyeIcon = document.getElementById('balance_eye_icon');
    const rawValue = displayDiv.getAttribute('data-raw-balance');

    if (displayDiv.innerText === rawValue) {
        displayDiv.innerText = '••••••••';
        eyeIcon.innerText= '🙈';
        localStorage.setItem('absar_wallet_hidden', 'true');
    } else {
        displayDiv.innerText = rawValue;
        eyeIcon.innerText = '👁️';
        localStorage.setItem('absar_wallet_hidden', 'false');
    }
}

function copyReferralLink() {
    const linkField = document.getElementById("referral_link_field");
    linkField.select();
    navigator.clipboard.writeText(linkField.value);
    alert("📋 لینک اختصاصی دعوت شما کپی شد!");
}

function downloadReferralQr() {
    const qrCanvas = document.querySelector("#referral_qrcode canvas");
    if (!qrCanvas) {
        alert("QR کد هنوز آماده نشده است.");
        return;
    }
    const link = document.createElement("a");
    link.download = "absar-referral-qr.png";
    link.href = qrCanvas.toDataURL("image/png");
    link.click();
}

document.addEventListener("DOMContentLoaded", function() {
    if (typeof QRCode !== "undefined" && document.getElementById("referral_qrcode")) {
        new QRCode(document.getElementById("referral_qrcode"), {
            text: referralLink,
            width: 140,
            height: 140,
            colorDark: "#0f172a",
    ...
    if (localStorage.getItem('absar_wallet_hidden') === 'true') {
        const displayDiv = document.getElementById('wallet_balance_display');
        const eyeIcon = document.getElementById('balance_eye_icon');
        if(displayDiv && eyeIcon) {
            displayDiv.innerText = '••••••••';
            eyeIcon.innerText = '🙈';
        }
    }
